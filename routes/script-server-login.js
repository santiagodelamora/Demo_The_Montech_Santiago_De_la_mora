/*
    ...
    Nombre del archivo: script-server-login.js
        ...
        case 'sign up form':
            ...
            database.query(sql, [receivedData[0], receivedData[1], receivedData[2], new Date()], (error, result) => {
                if (error)
                {
                    ...
                    return response.status(500).json({success: false, message: "Error al insertar los datos en la tabla."});
                }
        ...
        case 'sign in form':
            ...
            database.query(sql, [receivedData[0]], (error, results) => {
                if (error)
                {
                    ...
                    return response.status(500).json({success: false, message: "Error al consultar los datos de la tabla."});
                }
                
                if (results.length < 1) {
                    return response.status(401).json({success: false, message: "Correo electrónico no encontrado."});
                }
                ...
                // Envía una respuesta exitosa de inicio de sesión
                ...
                // Ejecuta la consulta creada anteriormente. Si hay algún error durante la inserción, se envía una respuesta con estado 500 (error del servidor) y un mensaje JSON al cliente.
                // En caso contrario, se envíar una respuesta con estado 200 (OK) y se envía un mensaje al cliente
                database.query(sql, [receivedData[0], receivedData[1], new Date()], (insertError, insertResult) => {
                    if (insertError)
                    {
                        console.error("Error al insertar los datos en la tabla.", insertError);
